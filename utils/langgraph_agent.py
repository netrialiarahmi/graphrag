...
def _assemble_context_for_state(state: GraphState, raw_vdb_hits=None) -> GraphState:
    ...
    if neo4j_client.test_connection():
        # Keep only docs that actually exist in Neo4j to avoid alias/hallucinated IDs.
        valid_doc_ids = []
        for did in doc_ids:
            try:
...
def router_node(state: GraphState) -> GraphState:
    ...
    if regex_ids and neo4j_client.test_connection():
        # Validate early so direct route is not triggered by non-existent doc IDs.
        valid_regex_ids = set()
    ...
    try:
        ...
        if get_logging_config().get("verbose", False):
            log_event(
                ...
                stage="router",
                event="prompt_input",
                payload={"system_prompt": system_prompt, "user_prompt": user_content},
                trim_payload=False,
            )
...
def direct_lookup_node(state: GraphState) -> GraphState:
    ...
    if doc_ids and neo4j_client.test_connection():
        # Validate that selected doc IDs actually exist in Neo4j.
        valid_doc_ids = []
        for did in doc_ids:
            try:
                if neo4j_client.get_document_detail(did):
...
def semantic_search_node(state: GraphState) -> GraphState:
    ...
    log_event(
        "graphrag.app",
        "Semantic retrieval candidates built",
        trace_id=trace_id,
        route="semantic",
        stage="semantic_search",
        event="retrieval_candidates",
        payload={"raw_hit_count": len(raw), "selected_doc_ids": doc_ids},
    )
    ...
    log_event(
        "graphrag.app",
        "Semantic LLM rerank completed",
        trace_id=trace_id,
        route="semantic",
        stage="semantic_rerank",
        event="rerank_result",
        payload={"candidate_chunks": len(all_candidates), "kept_chunks": len(reranked)},
    )
...
def deep_research_node(state: GraphState) -> GraphState:
    ...
    query = state["query"]
    expanded = llm_stance.expand_query(query, trace_id=trace_id, route="deep")
    ...
    ranked = llm_stance.rerank_documents(query, doc_summaries, trace_id=trace_id, route="deep")
...
def generate_answer_node(state: GraphState) -> GraphState:
    ...
    if get_logging_config().get("verbose", False):
        log_event(
            "graphrag.debug",
            "Answer node context payload",
            trace_id=trace_id,
            route=state.get("route", "unknown"),
            stage="generate_answer",
            event="context_chunks",
            payload={"chunks": chunks, "answer": ans},
            trim_payload=False,
        )
...
def summarize_if_needed_node(state: GraphState) -> GraphState:
    ...
    try:
        ...
        log_event(
            ...
            duration_ms=(time.perf_counter() - start) * 1000,
            payload={"condensed_messages": len(older), "kept_messages": len(kept)},
        )
    except Exception as e:
